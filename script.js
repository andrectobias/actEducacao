document.addEventListener("DOMContentLoaded", () => {
  ...
  /**
   * Fallback para celulares/tablets (e navegadores sem suporte à File System
   * Access API): abre o seletor de arquivo HTML padrão e lê o conteúdo via
   * FileReader, já que showOpenFilePicker não está disponível nesses casos.
   */
  const loadDataFromFileMobile = () => {
    return new Promise((resolve) => {
      ...
      input.addEventListener("change", () => {
        ...
        reader.onload = (e) => {
          try {
            ...
            CustomSwal.fire(
              "Erro",
              "Não foi possível ler o arquivo selecionado. Verifique se é um arquivo .json válido gerado pelo actEducação.",
              "error",
            );
            resolve(false);
          }
      ...
      // Em alguns navegadores móveis o seletor só é disparado com o input no DOM.
  ...
  const loadDataFromFile = async () => {
    if (!useNativeFilePicker) {
      return await loadDataFromFileMobile();
    }
   ...
   * Em celulares/tablets não existe seletor "Salvar como...", então o app
   * inicia com os dados padrão e ativa o salvamento automático no
  ...
  const createNewFile = async () => {
    if (!useNativeFilePicker) {
      ...
      CustomSwal.fire({
        ...
        icon: "success",
        title:
          "Novo arquivo criado! Os dados serão salvos automaticamente neste dispositivo.",
        showConfirmButton: false,
        timer: 3000,
      });
      return true;
  ...
  const promptToLoadOrCreateFile = () => {
    const autoSaveNote = useNativeFilePicker
      ? "Seu trabalho será salvo automaticamente no arquivo escolhido."
      : "Como você está em um celular/tablet, seus dados serão salvos automaticamente neste dispositivo. Use \"Exportar para Arquivo\" no menu de Backup sempre que quiser guardar uma cópia .json.";
  ...
  const init = async () => {
    ...
    // Em celulares/tablets (ou navegadores sem suporte à File System Access API),
    // se já existirem dados salvos automaticamente neste dispositivo em uma
    // visita anterior, continua direto de onde parou em vez de pedir para
    // selecionar o arquivo novamente.
    const hasStoredAutoSaveData =
      !useNativeFilePicker && !!localStorage.getItem(AUTO_SAVE_STORAGE_KEY);
